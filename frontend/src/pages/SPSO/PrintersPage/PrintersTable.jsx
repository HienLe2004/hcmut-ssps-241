...
export const PrintersTable = ({printers}) => {
    ...
    const updateStatus = () => {
        fetch(`/api/printers/${currentPrinter.id}`, {
            method: 'PATCH',
            body: JSON.stringify({status: currentPrinter.status=="on"?"off":"on"})
        })
        .then((res)=>res.json())
        .then(json=>console.log(json))
        printers.forEach(printer => {if (printer.id == currentPrinter.id) printer.status = printer.status=="on"?"off":"on"})
    }
    return <div className="w-full">
        ...
        {updateOpen && <UpdatePrinterStatus turnOn={currentPrinter.status=="off"} closeUpdate = {() => {setUpdateOpen(false)}} openNoti={() => {updateStatus() ;setNotiOpen(true)}}/>}
        {notiOpen && <Notification noti={"Đã xác nhận "+ (currentPrinter.status == "on" ? "bật":"tắt") +" máy thành công!"} closeNoti={()=>setNotiOpen(false)}/>}
    </div>
}